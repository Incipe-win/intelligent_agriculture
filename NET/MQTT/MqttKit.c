...
//	说明：
//==========================================================
uint8 MQTT_PacketConnect(const int8 *user, const int8 *password,
                         const int8 *devid, uint16 cTime, uint1 clean_session,
                         uint1 qos, const int8 *will_topic,
                         const int8 *will_msg, int32 will_retain,
...
//	说明：
//==========================================================
uint1 MQTT_PacketSaveData(const int8 *devid, int16 send_len,
                          int8 *type_bin_head, uint8 type,
                          MQTT_PACKET_STRUCTURE *mqttPacket) {

  if (MQTT_PacketPublish(MQTT_PUBLISH_ID, "$dp", NULL, send_len + 3,
...
uint1 MQTT_PacketSaveBinData(const int8 *name, int16 file_len,
  ...
  if (MQTT_PacketPublish(MQTT_PUBLISH_ID, "$dp", payload, payload_size,
                         MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
    result = 0;
...
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req,
  ...
  *cmdid =
      (int8 *)MQTT_MallocBuffer(37); // cmdid固定36字节，多分配一个结束符的位置
  if (*cmdid == NULL)
    return 2;
  ...
  *req_len =
      remain_len -
      44; //命令长度 = 剩余长度(remain_len) - 2 - 5($creq) - 1(\) - cmdid长度
...
uint1 MQTT_PacketCmdResp(const int8 *cmdid, const int8 *req,
  ...
  if (MQTT_PacketPublish(MQTT_PUBLISH_ID, payload, req, strlen(req),
...
uint8 MQTT_PacketSubscribe(uint16 pkt_id, enum MqttQosLevel qos,
  ...
  // 2 bytes packet id + topic filter(2 bytes topic + topic length + 1 byte
  // reserve)------
...
uint8 MQTT_UnPacketSubscribe(uint8 *rev_data) {
  ...
  if (rev_data[2] == MOSQ_MSB(MQTT_SUBSCRIBE_ID) &&
      rev_data[3] == MOSQ_LSB(MQTT_SUBSCRIBE_ID)) {
    switch (rev_data[4]) {
    case 0x00:
    case 0x01:
    case 0x02:
...
//	说明：
//==========================================================
uint8 MQTT_PacketUnSubscribe(uint16 pkt_id, const int8 *topics[],
                             uint8 topics_cnt,
                             MQTT_PACKET_STRUCTURE *mqttPacket) {
  ...
  // 2 bytes packet id, 2 bytes topic length + topic + 1 byte
  // reserve---------------------
...
uint1 MQTT_UnPacketUnSubscribe(uint8 *rev_data) {
  ...
  if (rev_data[2] == MOSQ_MSB(MQTT_UNSUBSCRIBE_ID) &&
      rev_data[3] == MOSQ_LSB(MQTT_UNSUBSCRIBE_ID)) {
    result = 0;
  }
...
//	说明：
//==========================================================
uint8 MQTT_PacketPublish(uint16 pkt_id, const int8 *topic, const int8 *payload,
                         uint32 payload_len, enum MqttQosLevel qos,
                         int32 retain, int32 own,
                         MQTT_PACKET_STRUCTURE *mqttPacket) {
  ...
  if (payload != NULL) {
    if (payload[0] == 2) {
      ...
      memcpy((int8 *)mqttPacket->_data + mqttPacket->_len, payload,
             payload_len);
...
uint8 MQTT_UnPacketPublish(uint8 *rev_data, int8 **topic, uint16 *topic_len,
                           int8 **payload, uint16 *payload_len, uint8 *qos,
                           uint16 *pkt_id) {
...
uint1 MQTT_UnPacketPublishAck(uint8 *rev_data) {
  ...
  if (rev_data[2] == MOSQ_MSB(MQTT_PUBLISH_ID) &&
      rev_data[3] == MOSQ_LSB(MQTT_PUBLISH_ID))
    return 0;
  else
    return 1;
}
