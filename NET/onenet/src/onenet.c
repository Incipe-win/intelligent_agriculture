 ...
 *
 *	修改记录：
 *V1.0：协议封装、返回判断都在同一个文件，并且不同协议接口不同。
...
_Bool OneNet_DevLink(void) {
  ...
  if (MQTT_PacketConnect(PROID, AUTH_INFO, DEVID, 256, 0, MQTT_QOS_LEVEL0, NULL,
                         NULL, 0, &mqttPacket) == 0) {
...
void OneNet_Subscribe(const char *topics[], unsigned char topic_cnt) {
  ...
  if (MQTT_PacketSubscribe(MQTT_SUBSCRIBE_ID, MQTT_QOS_LEVEL0, topics,
                           topic_cnt, &mqttPacket) == 0) {
...
void OneNet_RevPro(unsigned char *cmd) {
  ...
  cJSON *json = NULL, *json_led2 = NULL, *json_beep = NULL;
  ...
  case MQTT_PKT_CMD: //命令下发
    ...
    if (result == 0) {
      ...
      if (MQTT_PacketCmdResp(cmdid_topic, req_payload, &mqttPacket) ==
          0) //命令回复组包
      {
  ...
  case MQTT_PKT_PUBLISH: //接收的Publish消息
    ...
    result = MQTT_UnPacketPublish(cmd, &cmdid_topic, &topic_len, &req_payload,
                                  &req_len, &qos, &pkt_id);
    if (result == 0) {
      UsartPrintf(USART_DEBUG,
                  "topic: %s, topic_len: %d, payload: %s, payload_len: %d\r\n",
      ...
      if (!json) {
        ...
        UsartPrintf(USART_DEBUG, "json_led2 = %d\r\n", json_led2);
        UsartPrintf(USART_DEBUG, "json_beep = %d\r\n", json_beep);
        if (json_led2->valueint) {
          LED2 = 0;
        } else {
          LED2 = 1;
        }
        if (json_beep->valueint) {
          BEEP = 0;
          flag = 0;
        } else {
          BEEP = 1;
